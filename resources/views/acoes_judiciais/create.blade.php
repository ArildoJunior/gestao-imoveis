<?php // resources/views/acoes_judiciais/create.blade.php ?>

@extends('layouts.app')

@section('title', 'Registrar Nova Ação Judicial')

@section('content')
    <h2 class="mb-4">Registrar Nova Ação Judicial</h2>
    ...
    <div class="card mb-4">
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('acoes-judiciais.store') }}">
                @csrf
                    ...
                    <select id="contrato_id" name="contrato_id" class="form-select" required>
                        ...
                        @foreach($contratos as $contrato)
                            <option value="{{ $contrato->id }}"
                                {{ old('contrato_id', $contratoSelecionado?->id ?? '') == $contrato->id ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <select id="imovel_devolvido" name="imovel_devolvido" class="form-select" required>
                        <option value="0" {{ old('imovel_devolvido') == '0' ? 'selected' : '' }}>Não</option>
                        <option value="1" {{ old('imovel_devolvido') == '1' ? 'selected' : '' }}>Sim</option>
                    </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="condicao_imovel_entrega" name="condicao_imovel_entrega" class="form-select">
                                ...
                                @foreach($options['condicoesImovel'] as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ old('condicao_imovel_entrega') == $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="mb-3">
                    ...
                    <select id="houve_acordo" name="houve_acordo" class="form-select" required>
                        <option value="0" {{ old('houve_acordo') == '0' ? 'selected' : '' }}>Não</option>
                        <option value="1" {{ old('houve_acordo') == '1' ? 'selected' : '' }}>Sim</option>
                    </select>
                    @error('houve_acordo')
                        <div class="text-danger mt-1">{{ $message }}</div>
                    @enderror
                ...
                <div id="bloco_acordo" class="mb-3 p-3 border rounded" style="display: none;">
                    <div class="mb-3">
                        ...
                        <textarea id="descricao_acordo" name="descricao_acordo" class="form-control">{{ old('descricao_acordo') }}</textarea>
                ...
                <div class="mb-3">
                    ...
                    <select id="novo_contrato_apos_decisao" name="novo_contrato_apos_decisao" class="form-select" required>
                        <option value="0" {{ old('novo_contrato_apos_decisao') == '0' ? 'selected' : '' }}>Não</option>
                        <option value="1" {{ old('novo_contrato_apos_decisao') == '1' ? 'selected' : '' }}>Sim</option>
                    </select>
    ...
    {{-- Inclui o script JavaScript via @push('scripts') --}}
    @push('scripts')
        <script>
